...
- [서비스별 상세 PDF 설명서 (26페이지)](reports/user-guide/operation-cns-elect-user-guide.pdf)
...
현재 6개 메뉴를 독립 챕터로 구분하고 기능 요약·유사 서비스와의 차이,
실제 실행 화면, 입력 형식, 단계별 사용법, 결과 예시와
...
- 독립된 점등 내역 편집 페이지는 삭제했으며, 기존 `/light-log` 주소는 이 화면으로 연결
- 편집·보고 탭에서 같은 파일을 사용하고 원본 대비 신규 점등·소등을 담당자별로 보고
...
| `npm run test:light-log`           | 통합 화면의 공용 점등 내역 계산·복원 테스트 |
...
점검대상 칸의 구역별 내용을 `/`로 이어 붙인 것이 기존 내역입니다. 신규 점등은
`A-15,21`처럼 쉼표나 줄바꿈으로 구분하며, `A-15(3)`은 같은 자리 3개를 뜻합니다.
소등은 입력하지 않고 `점등장비` 목록에서 행을 골라
...
- 각 줄은 `[기능페이지] 작업 내용` 형식으로 작성하고 줄바꿈 없이 표시
- 날짜는 일수 없이 `YYYY.MM`만 사용
- 작업자 이름·직급이나 작성자 꼬리표는 표시하지 않음
