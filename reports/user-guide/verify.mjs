...
for (const [i, page] of pages.entries()) {
  ...
  assert.match(page, new RegExp(`${String(i + 1).padStart(2, '0')}\\s*[/]\\s*20`));
}
...
try {
  ...
  for (let group = 0; group < 4; group++) {
    const images = Array.from({ length: 5 }, (_, i) => group * 5 + i + 1).map(n => `<figure><img src="${new URL(`./pdf-preview/page-${String(n).padStart(2, '0')}.png`, import.meta.url).href}"><figcaption>Page ${n}</figcaption></figure>`).join('');
...
console.log('PDF 한글·20페이지·꼬릿말, XLSX 4행·필터, ZIP 구성·PDF 일치 확인 완료');
