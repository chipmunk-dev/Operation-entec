...
function LightLogEditor() {
  ...
  return (
    <div className="page-shell">
      ...
      <section
        className="relative mb-6 overflow-hidden rounded-3xl border border-rose-200 bg-gradient-to-br from-rose-50 via-white to-amber-50 p-6 shadow-sm shadow-rose-100/70 sm:p-7"
        role="alert"
      >
        <span
          aria-hidden="true"
          className="absolute -right-10 -top-14 h-40 w-40 rounded-full bg-rose-200/35 blur-3xl"
        />
        <div className="relative flex items-start gap-4">
          <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-rose-600 text-2xl text-white shadow-lg shadow-rose-200">
            <MdLockOutline aria-hidden="true" />
          </span>
          <div className="min-w-0">
            <span className="status-pill mb-2 bg-rose-100 text-rose-700">
              기능 사용 일시 중단
            </span>
            ...
            <p className="mt-3 max-w-3xl text-sm font-medium leading-6 text-slate-600 sm:text-base">
              해당 페이지는 양식 변경 진행으로 인하여, 현재 기능을 사용할 수
              없습니다. 양식 변경 확정 및 안정화가 되면 이후 필요시 기능을
              리팩토링하거나 제거할 예정입니다.
            </p>
          </div>
        </div>
      ...
      >
        <div className="pointer-events-none absolute inset-x-0 top-3 z-20 flex justify-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-slate-900/90 px-4 py-2 text-xs font-bold text-white shadow-lg backdrop-blur">
            <MdLockOutline aria-hidden="true" />
            편집 기능 잠김
          </span>
        </div>

        <div className="pointer-events-none select-none opacity-40 grayscale-[0.25] saturate-50">
          <div
          ...
          <div className="grid gap-6">
            ...
            <section className="panel overflow-hidden">
              ...
              <div className="panel-body">
                ...
                <p className="mt-3 text-xs text-slate-500">
                  쉼표나 줄바꿈으로 구분합니다.{' '}
                  <code className="font-mono">A-1,2,3</code>처럼 적으면 A-1 ·
                  A-2 · A-3으로 읽습니다. 같은 자리가 여러 개면{' '}
                  <code className="font-mono">D-24(2)</code>처럼 개수를
                  적습니다.
                </p>
              </div>
            </section>
            ...
            <section className="panel flex min-h-[360px] flex-col overflow-hidden">
              ...
              <div className="flex-1 p-5">
                ...
                {log && !log.hasHead && (
                  <p className="mb-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-semibold text-amber-800">
                    머리말의 개수 표기를 찾지 못했습니다. 개수는 세었지만
                    갱신하지 않고 원문을 그대로 두었습니다.
                  </p>
                )}
                {copyFailed && (
                  <p className="mb-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-xs font-semibold text-rose-700">
                ...
                {canCopy ? (
                  <pre
                    ...
                    dangerouslySetInnerHTML={{
                      __html: colorizeOutput(log.output),
                    }}
                  />
                ) : (
